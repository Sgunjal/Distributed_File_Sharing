...
public class Client1_Test extends Thread
{
	...
	File file_list=null;
	Client1_Test(String type)								// Constructor
	{
		this.type=type;
	}
	Client1_Test(Socket s,String type)						/// Constructor 
	{
		this.type=type;
		incoming_socket=s;
	}
	...
	{		
		try
		{
			...
			{
				ServerSocket ss=new ServerSocket(25001);		// Create server socket on port 25002 which will listen to incoming socket request
				type="Request";									// apply requester type to incoming connections
				while(true)
				{
					 Socket s = ss.accept();  		            // accpet incoming request
					 Thread t1 =new Client1_Test(s,"Request");	// Create separate thread for each incoming request.
					 t1.start();
				}				
			}
			else
	...
	//		value=c_put.readLine();
			...
			//	System.out.println("Key & Value sent to server to store");
				...
				s_out2_array1[index].println("1");                                         // Send value to server 
	...
	{												
		...
		{			
			...
			Client1_Test ct=new Client1_Test("Client");									// Create client object 
			Thread t=new Client1_Test("Server");										// Create server thread to accept incoming socket
