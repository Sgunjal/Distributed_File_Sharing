...
public class Client5_Test extends Thread
{
	...
	File file_list=null;
	Client5_Test(String type)								// Constructor
	{
		this.type=type;
	}
	Client5_Test(Socket s,String type)						/// Constructor 
	{
		this.type=type;
		incoming_socket=s;
	}
	...
	{		
		try
		{
			...
			{
				ServerSocket ss=new ServerSocket(25005);		// Create server socket on port 25002 which will listen to incoming socket request
				type="Request";									// apply requester type to incoming connections
				while(true)
				{
					 Socket s = ss.accept();  		            // accpet incoming request
					 Thread t1 =new Client5_Test(s,"Request");	// Create separate thread for each incoming request.
					 t1.start();
				}				
			}
			else
	...
	//		value=c_put.readLine();
			...
			try
			{
				...
				s_out2_array1[index].println("5");                                         // Send value to server 
	...
	{												
		...
		{			
			...
			Client5_Test ct=new Client5_Test("Client");									// Create client object 
			Thread t=new Client5_Test("Server");										// Create server thread to accept incoming socket
