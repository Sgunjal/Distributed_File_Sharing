...
public class Client8_Test extends Thread
{
	...
	File file_list=null;
	Client8_Test(String type)								// Constructor
	{
		this.type=type;
	}
	Client8_Test(Socket s,String type)						/// Constructor 
	{
		this.type=type;
		incoming_socket=s;
	}
	...
	{		
		try
		{
			...
			{
				ServerSocket ss=new ServerSocket(25008);		// Create server socket on port 25002 which will listen to incoming socket request
				type="Request";									// apply requester type to incoming connections
				while(true)
				{
					 Socket s = ss.accept();  		            // accpet incoming request
					 Thread t1 =new Client8_Test(s,"Request");	// Create separate thread for each incoming request.
					 t1.start();
				}				
			}
			else
	...
	//		value=c_put.readLine();
			...
			try
			{
				...
				s_out2_array1[index].println("8");                                         // Send value to server 
	...
	{												
		...
		{			
			...
			Client8_Test ct=new Client8_Test("Client");									// Create client object 
			Thread t=new Client8_Test("Server");										// Create server thread to accept incoming socket
