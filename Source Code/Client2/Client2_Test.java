...
public class Client2_Test extends Thread
{
	...
	//		value=c_put.readLine();
			...
			try
			{
				if(flag==1)
				{
					s_out2_array_bkp[connect_server].println("1");							// send option 1 to server 
					s_out2_array_bkp[connect_server].println(key);                           // Send key to server 
					s_out2_array_bkp[connect_server].println(connect_server);                          // Send value to server 					
